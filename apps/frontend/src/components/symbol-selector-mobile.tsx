...
export const SymbolSelectorMobile = forwardRef<HTMLButtonElement, SymbolSelectorMobileProps>(
  (
    ...
    const displayText = selectedSymbol
      ...
      : value || placeholder || t("common.selectSymbol");
            ...
            {showCustomAssetForm ? (
                ...
                <SheetHeader className="border-border space-y-0 border-b px-4 py-4">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex-1">
                      <SheetTitle>{t("assets.createCustomAsset")}</SheetTitle>
                      <SheetDescription>{t("assets.createCustomAssetDesc")}</SheetDescription>
                    </div>
                  </div>
                </SheetHeader>
                  ...
                  <ScrollArea className="flex-1">
                    <div className="form-mobile-spacing px-6 py-4">
                      ...
                      <div className="space-y-2">
                        <label className="text-base font-medium">{t("assets.symbolTicker")}</label>
                        <Input
                      ...
                      {/* Name */}
                      <div className="space-y-2">
                        <label className="text-base font-medium">{t("assets.name")}</label>
                        <Input
                          placeholder={t("assets.customNamePlaceholder")}
                          value={customName}
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="space-y-2">
                          <label className="text-base font-medium">{t("assets.assetType")}</label>
                          <Select
                        ...
                        <div className="space-y-2">
                          <label className="text-base font-medium">{t("assets.currency")}</label>
              ...
              <>
                ...
                <SheetHeader className="border-border border-b px-6 py-4">
                  <SheetTitle>{t("common.selectSymbol")}</SheetTitle>
                  <SheetDescription>{t("assets.addSecurityDesc")}</SheetDescription>
                </SheetHeader>
                  ...
                  <ScrollArea className="flex-1 px-6 py-4">
                    ...
                    {searchQuery.length === 0 && (
                      <div className="text-muted-foreground flex h-full flex-col items-center justify-center gap-2 text-center text-sm">
                        ...
                        <p>{t("assets.startTypingToSearch")}</p>
                      </div>
                    )}
                    ...
                    {searchQuery.length > 0 &&
                      ...
                      searchQuery.length > 1 && (
                        <div className="text-muted-foreground py-8 text-center text-sm">
                          <p>{t("assets.noMatchesFor", { query: searchQuery })}</p>
                          <p className="mt-2 text-xs">{t("assets.canCreateCustomBelow")}</p>
                        </div>
                      )}
                    ...
                    {searchQuery.length > 0 && searchQuery.length <= 1 && (
                      <div className="text-muted-foreground flex h-full items-center justify-center text-sm">
                        {t("assets.typeAtLeastTwoCharacters")}
                      </div>
                    )}
                        ...
                        >
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="text-foreground font-medium">
                              {t("assets.createCustomAsset")}
                            </div>
                            <div className="text-muted-foreground mt-0.5 text-sm">
                              {searchQuery.trim()
                                ? t("assets.createWithManualPricing", {
                                    symbol: searchQuery.trim().toUpperCase(),
                                  })
                                : t("assets.trackAssetsNotFound")}
                            </div>
                          </div>
                          <Icons.ChevronRight className="text-muted-foreground h-5 w-5 shrink-0" />
                        </button>
