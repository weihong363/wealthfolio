...
function ActivitySummaryRow({
  ...
  const symbol = normalized.assetSymbol || normalized.assetId || t("activities.cash");
              ...
              <div className="text-muted-foreground flex flex-wrap gap-1 text-[11px]">
                {candidate.reasons.map((reason) => (
                  <Badge key={reason} variant="secondary" className="rounded-sm px-1.5 py-0">
                    {t(`activities.transferMatch.reasons.${reason}`, { defaultValue: reason })}
                  </Badge>
                ))}
              </div>
            ) : null}
...
export function TransferMatchDialog({
  ...
  const title =
    mode === "link"
      ? t("activities.transferMatch.linkTitle")
      : t("activities.transferMatch.unlinkTitle");
  const description =
    mode === "link"
      ? t("activities.transferMatch.linkDescription")
      : canClearInvalidLink
        ? t("activities.transferMatch.clearStaleDescription")
        : t("activities.transferMatch.unlinkDescription");
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto py-4">
          <div className="space-y-2">
            <div className="text-muted-foreground text-xs font-medium uppercase">
              {t("activities.transferMatch.selectedRow")}
            </div>
            <ActivitySummaryRow
          ...
          {mode === "unlink" ? (
            <div className="space-y-3">
              <div className="text-muted-foreground text-xs font-medium uppercase">
                {t("activities.transferMatch.linkedCounterpart")}
              </div>
              {counterpartLoading ? (
                <div className="text-muted-foreground rounded-md border p-3 text-sm">
                  {t("activities.transferMatch.loadingLinked")}
                </div>
              ) : counterpart ? (
                <ActivitySummaryRow
                ...
                <div className="rounded-md border p-3 text-sm">
                  <div className="font-medium">
                    {t("activities.transferMatch.noCounterpart")}
                  </div>
                  <div className="text-muted-foreground mt-1">
                    {t("activities.transferMatch.clearStaleHint")}
                  </div>
                </div>
              ) : (
                <div className="text-destructive rounded-md border p-3 text-sm">
                  {counterpartError ?? t("activities.transferMatch.noCounterpart")}
                </div>
              )}
            </div>
          ) : (
            <>
              <div className="space-y-2">
                <div className="flex items-center justify-between gap-2">
                  <div className="text-muted-foreground text-xs font-medium uppercase">
                    {t("activities.transferMatch.suggestedMatches")}
                  </div>
                  {oppositeType ? (
                    <div className="flex items-center gap-1.5">
                      <span className="text-muted-foreground text-xs">
                        {t("activities.transferMatch.lookingFor")}
                      </span>
                ...
                {suggestionsLoading ? (
                  ...
                  <div className="flex flex-col items-center gap-2 rounded-md border border-dashed px-4 py-6 text-center">
                    ...
                    <p className="text-muted-foreground text-sm">
                      {t("activities.transferMatch.noSevenDayMatches")}
                    </p>
                    <Button
                      type="button"
                      variant="outline"
                    ...
                    >
                      {t("activities.transferMatch.searchThirtyDays")}
                    </Button>
                  </div>
                )}
              </div>
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="min-w-0">
                    <div className="text-sm font-medium">
                      {t("activities.transferMatch.searchEligible")}
                    </div>
                    <div className="text-muted-foreground text-xs">
                      {t("activities.transferMatch.searchScope", {
                        type: oppositeType
                          ? (ActivityTypeNames[oppositeType] ?? oppositeType)
                          : t("activities.transferMatch.oppositeTransfer"),
                      })}
                    </div>
                  </div>
                ...
                >
                  <div className="flex gap-2">
                    <div className="relative flex-1">
                      ...
                      <Input
                        ...
                        placeholder={t("activities.transferMatch.searchPlaceholder")}
                        className="pl-9"
                      />
                    </div>
                  ...
                  <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
                    <Select value={accountId} onValueChange={setAccountId}>
                      <SelectTrigger>
                        <SelectValue placeholder={t("activities.table.account")} />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value={ALL}>{t("activities.filters.allAccounts")}</SelectItem>
                    ...
                    <Select value={windowDays} onValueChange={setWindowDays}>
                      <SelectTrigger>
                        <SelectValue placeholder={t("activities.transferMatch.dateRange")} />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="7">
                          {t("activities.transferMatch.withinDays", { count: 7 })}
                        </SelectItem>
                        <SelectItem value="30">
                          {t("activities.transferMatch.withinDays", { count: 30 })}
                        </SelectItem>
                        <SelectItem value="90">
                          {t("activities.transferMatch.withinDays", { count: 90 })}
                        </SelectItem>
                        <SelectItem value="180">
                          {t("activities.transferMatch.withinDays", { count: 180 })}
                        </SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  ...
                  <Collapsible open={filtersOpen} onOpenChange={setFiltersOpen}>
                    <CollapsibleTrigger asChild>
                      ...
                      >
                        ...
                        />
                        {t("activities.transferMatch.moreFilters")}
                    ...
                    <CollapsibleContent className="pt-2">
                      <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
                        <Select value={accountType} onValueChange={setAccountType}>
                          <SelectTrigger>
                            <SelectValue placeholder={t("activities.transferMatch.accountType")} />
                          </SelectTrigger>
                          <SelectContent>
                            <SelectItem value={ALL}>
                              {t("activities.transferMatch.allAccountTypes")}
                            </SelectItem>
                        ...
                        <Select value={currency} onValueChange={setCurrency}>
                          <SelectTrigger>
                            <SelectValue placeholder={t("activities.table.currency")} />
                          </SelectTrigger>
                          <SelectContent>
                            <SelectItem value={ALL}>
                              {t("activities.transferMatch.allCurrencies")}
                            </SelectItem>
                        ...
                        <Select value={assetFilter} onValueChange={setAssetFilter}>
                          <SelectTrigger>
                            <SelectValue placeholder={t("fundResearch.asset")} />
                          </SelectTrigger>
                          <SelectContent>
                            <SelectItem value={ALL}>
                              {t("activities.transferMatch.allAssets")}
                            </SelectItem>
                            {isSecurityTransfer(source) ? (
                              <SelectItem value={SAME_ASSET}>
                                {t("activities.transferMatch.sameAsset")}
                              </SelectItem>
                            ) : (
                              <SelectItem value={CASH_ASSET}>
                                {t("activities.transferMatch.cashOnly")}
                              </SelectItem>
                            )}
                          </SelectContent>
                        </Select>
                        ...
                        <Select value={matchMode} onValueChange={setMatchMode}>
                          <SelectTrigger>
                            <SelectValue placeholder={t("activities.transferMatch.matchMode")} />
                          </SelectTrigger>
                          <SelectContent>
                            <SelectItem value={STRICT}>
                              {t("activities.transferMatch.exactMatch")}
                            </SelectItem>
                            <SelectItem value={ANY}>
                              {t("activities.transferMatch.anyEligible")}
                            </SelectItem>
                          </SelectContent>
                        </Select>
                      </div>
                  ...
                  <div className="space-y-2">
                    <div className="text-muted-foreground text-xs">
                      {filteredManualResults.length === 1
                        ? t("activities.transferMatch.eligibleCount", { count: 1 })
                        : t("activities.transferMatch.eligibleCount", {
                      ...
                      <div className="space-y-2 pr-3">
                        {filteredManualResults.length > 0 ? (
                          ...
                          <div className="text-muted-foreground rounded-md border border-dashed p-4 text-center text-sm">
                            {t("activities.transferMatch.noFilterMatches")}
                          </div>
                        )}
                      </div>
                    </ScrollArea>
        ...
        <SheetFooter>
          ...
          {mode === "link" ? (
            ...
            >
              ...
              {t("activities.transferMatch.linkTransfers")}
            </Button>
          ) : (
            <Button
            ...
            >
              ...
              {counterpart
                ? t("activities.transferMatch.unlinkTransfers")
                : t("activities.transferMatch.unlinkTransfer")}
            </Button>
          )}
        </SheetFooter>
      </SheetContent>
