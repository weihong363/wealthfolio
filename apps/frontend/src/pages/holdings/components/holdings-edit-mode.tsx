...
export const HoldingsEditMode = ({
  ...
  const handleAddHolding = useCallback(
    (_symbol: string, searchResult?: SymbolSearchResult) => {
      ...
      if (editableHoldings.some((h) => h.assetId === assetId)) {
        toast.error(t("holdings.holdingAlreadyExists"));
        return;
      }
  ...
  const handleSelectCashCurrency = useCallback(
    (currency: string) => {
      ...
      if (cashBalances.some((c) => c.currency === currency)) {
        toast.error(t("holdings.currencyAlreadyExists"));
        return;
      }
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      toast.success(t("holdings.holdingsUpdated"));
      onClose();
    } catch (error) {
      ...
      toast.error(
        typeof error === "string"
          ...
          : error instanceof Error
            ? error.message
            : t("holdings.failedToSaveHoldings"),
      );
    } finally {
      setIsSaving(false);
    }
  ...
  const handleDeleteSnapshot = useCallback(async () => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: [QueryKeys.CURRENT_VALUATION] });
      queryClient.invalidateQueries({ queryKey: QueryKeys.manualSnapshots(account.id) });
      toast.success(t("holdings.snapshotDeleted"));
      onClose();
    } catch (error) {
      console.error("Failed to delete snapshot:", error);
      toast.error(error instanceof Error ? error.message : t("holdings.failedToDeleteSnapshot"));
    } finally {
      setIsDeleting(false);
  ...
  return (
    <div className="flex h-full flex-col">
        ...
        <div className="space-y-6 py-4">
          ...
          <Card>
            <CardContent className="space-y-4 pt-4">
              {/* Snapshot Date */}
              <div className="pb-2">
                <Label className="text-sm font-medium">{t("holdings.snapshotDate")}</Label>
                <p className="text-muted-foreground mb-2 text-xs">
                  {isEditingExistingSnapshot
                    ? t("holdings.editingSnapshotDesc")
                    : t("holdings.snapshotDateDesc")}
                </p>
                <DatePickerInput
                  value={snapshotDate}
              ...
              <div className="text-muted-foreground grid grid-cols-12 gap-2 border-b pb-2 text-xs font-medium">
                ...
                <div className="col-span-2 text-right">{t("holdings.avgCost")}</div>
                  ...
                  <div className="flex items-center gap-2 py-2">
                    <div className="flex-1">
                      <TickerSearchInput
                        ...
                        placeholder={t("holdings.searchForSymbol")}
          ...
          <Card>
            <CardContent className="space-y-4 pt-4">
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="font-medium">{t("holdings.cashBalances")}</h4>
                  <p className="text-muted-foreground text-xs">{t("holdings.addCashByCurrency")}</p>
                </div>
              </div>
      ...
      <AlertDialog open={showDiscardDialog} onOpenChange={setShowDiscardDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("holdings.discardChanges")}</AlertDialogTitle>
            <AlertDialogDescription>
              You have unsaved changes. Are you sure you want to discard them?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>{t("holdings.keepEditing")}</AlertDialogCancel>
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("holdings.deleteSnapshot")}</AlertDialogTitle>
            <AlertDialogDescription>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel disabled={isDeleting}>{t("holdings.cancel")}</AlertDialogCancel>
            <AlertDialogAction
