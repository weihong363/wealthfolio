...
const AccountSummaryComponent = React.memo(
  ({
    ...
    const subText = isGroup
      ? t("dashboard.accounts.accountCount", { count: item.accountCount ?? 0 })
      : useAccountCurrency
    ...
    const warningMessages = hasBadData
      ? [
          item.trackingMode === "HOLDINGS"
            ? t("dashboard.accounts.returnUnavailableHoldings")
            : t("dashboard.accounts.returnUnavailableTransactions"),
        ]
      : [];
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-destructive text-sm font-medium">
                  {t("dashboard.accounts.failedToLoad")}
                </p>
                <p className="text-muted-foreground mt-1 break-words text-xs">
                  {errorAccounts?.message || t("dashboard.accounts.unexpectedError")}
                </p>
                <p className="text-muted-foreground mt-2 text-xs">
                  {t("dashboard.accounts.restartHint")}
                </p>
              </div>
            </div>
          </div>
      ...
      if (!combinedAccountViews || combinedAccountViews.length === 0) {
        return (
          <div className="border-border/50 bg-success/10 rounded-xl border p-6 text-center md:p-8">
            <p className="text-sm">{t("dashboard.accounts.noAccounts")}</p>
            <Link
              to="/settings/accounts"
              className="text-muted-foreground hover:text-foreground mt-2 inline-flex items-center gap-1 text-xs underline-offset-4 hover:underline"
            >
              {t("dashboard.accounts.addFirstAccount")}
    ...
    return (
      <div className="mb-4 w-full space-y-0">
        <div className="flex flex-row items-center justify-between gap-2 pb-2">
          <h2 className="text-sm font-semibold tracking-tight">{t("dashboard.accounts.title")}</h2>
          <Button
            variant="ghost"
            ...
            aria-label={
              accountsGrouped
                ? t("dashboard.accounts.listView")
                : t("dashboard.accounts.groupView")
            }
            title={
              accountsGrouped
                ? t("dashboard.accounts.switchToListView")
                : t("dashboard.accounts.switchToGroupView")
            }
          ...
          <div className="border-destructive/30 bg-destructive/5 mb-2 rounded-lg border p-3">
            <div className="flex items-start gap-2">
              ...
              <div className="min-w-0">
                <p className="text-destructive text-sm font-medium">
                  {t("dashboard.accounts.failedToLoadPerformance")}
                </p>
                <p className="text-muted-foreground mt-1 break-words text-xs">
                  {performanceError?.message || t("dashboard.accounts.performanceFallback")}
                </p>
              </div>
            </div>
          </div>
